...
- **Traditional enrolment** — user gets an *enrolment request* with no role; the
...
are held *pending* our checks before they can submit. Cost (per FAQ): redeeming
...
- **Step 2 — both providers, live-ready + Defra ID mock:**
  1. Reconstruct `defra-id/client.js` (from transcript) and port `entra/client.js`.
  2. Port `get-permissions.js` with mock enrolment states.
  3. Implement Hapi routes: sign-in, callback, organisation re-selection, sign-out.
  4. `@hapi/yar` session: persist `sub` + selected `currentRelationshipId`.
  5. Role-aware post-login redirect (applicant → registration; case officer → admin).
  6. Mock identities + sign-in pages; live-mode "not fully configured" guard (422).
  7. Tests (Vitest) + `.env.example` documenting the live vars.
